...
弹幕 **跨节点 fan-out** 不查 K8s Service，走 **分层 Fan-out 寻址**（① RID→NID @ chatroom → ② (RID,GID)→(SID,CID) @ websocket → ③ CID→fd）。详 [弹幕系统的名词解释.md](弹幕系统的名词解释.md) §5。
...
| 文档 | 内容 |
|------|------|
| [弹幕系统的名词解释.md](弹幕系统的名词解释.md) | **分层 Fan-out 寻址** + NID/RID/SID/CID/GID（§5 详述） |
| [ARCHITECTURE.md](ARCHITECTURE.md) | 技术架构；§3.3 分层 Fan-out 寻址 |
...
*速记：**分层 Fan-out 寻址** = ① RID→NID（chatroom）→ ② (RID,GID)→(SID,CID)（websocket）→ ③ CID→fd。↑ 发 = websocket→chatroom；↓ 收 = 上述链 + ACK 单播发送方。*
