# 09 · Server 订阅与 node 映射 rtmq_comm

> **源文件**：`src/clang/lib/rtmq/server/rtmq_comm.c`  
> **模块**：Server 订阅与 node 映射 rtmq_comm  
> **说明**：publish 依赖的 sub hash；link 鉴权；nid→哪个 rsvr 线程。  
...
> **层级**：Server · 路由表  
> **在 RTMQ 中的位置**：SUB 登记 + publish 遍历 + nid 落在哪个 rsvr  
> **总地图**：[00-总地图.md](00-总地图.md) · 上一篇 `06-rtmq_recv_api` · 下一篇 `11-rtmq_rsvr_part2`
...
| `rtmq_sub_list_alloc` | 见下方代码块 |
| `rtmq_sub_group_trav_dealloc_cb` | 见下方代码块 |
| `rtmq_sub_list_dealloc` | 见下方代码块 |
| `rtmq_sub_group_alloc` | 见下方代码块 |
| `rtmq_sub_group_dealloc` | 见下方代码块 |
| `rtmq_sub_node_alloc` | 见下方代码块 |
| `rtmq_sub_add` | 见下方代码块 |
| `rtmq_sub_del` | 见下方代码块 |
...
```c
// 文件: src/clang/lib/rtmq/server/rtmq_comm.c
#include "redo.h"
#include "rtmq_comm.h"
#include "rtmq_recv.h"
