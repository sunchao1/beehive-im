...
> **读法**：先通读本文一遍建立「有血有肉」的画面；再按文末 **§11 对照表** 打开对应编号文档。**入参/出参逐步对照** → [群聊故事线-逐步入参出参.md](群聊故事线-逐步入参出参.md)
...
### 1.1 两层数据壳（有 RTMQ 的步骤都长这样）

```text
RTMQ 头（02）: type=IM.cmd  nid  flag=1  length  chksum
    └── IM 帧: MesgHeader(52B) + protobuf（mesg.pb.go）
        Proxy 回调 16/17 收到的 data = 整段 IM 帧（已剥 RTMQ 头）
```

HTTP/Redis/ImGroup 步骤无 RTMQ 头；各步 **入参·出参** 见 [群聊故事线-逐步入参出参.md](群聊故事线-逐步入参出参.md)。
...
| 步 | 100001 在做什么 | RTMQ | **入参 → 出参** | 17 篇 |
|----|-----------------|------|-----------------|-------|
| 1.1 | `GET /im/register` | **无** | Query `uid=100001,nation,city,town` → JSON **`UsrSvrRegisterRsp{sid:900001,…}`** | — |
| 1.2 | `GET /im/iplist` | **无** | `uid,sid,type=2(WS)` → JSON **`IpListRsp{token, list:[ws:8002]}`** | — |

详表 → [群聊故事线-逐步入参出参.md §2](群聊故事线-逐步入参出参.md#2-阶段-1注册--iplist无-rtmq)

---

## 4. 阶段 2：100001 上线 ONLINE

| 步 | 位置 | 动作 | RTMQ | **入参 → 出参** | 17 篇 |
|----|------|------|------|-----------------|-------|
| 2.1 | 浏览器 | WS 握手 `/im` | **无** | URL → **`LsndConnExtra{cid=1001}`** | — |
| 2.2 | 浏览器 | 发 `CMD_ONLINE 0x0101` | **无** | **`MesgHeader+MesgOnline{uid,sid,token}`** | — |
| 2.3 | ws-1 | `LsndMesgOnlineHandler` | **无** | IM 帧 → 补 **`Cid=1001,Nid=20001`** | — |
| 2.4 | ws-1 Proxy | **P-send** ONLINE | **Proxy** | IM 帧 → **RTMQ EXP 0x0101 包** | **13→15→02** |
| 2.5 | FORWARD | **S-pub** | **Server** | RTMQ 包 → 查 **09** 投 30000 | **10→12→06→09** |
| 2.6 | BACKEND | **16** handler | **P+S** | `data=IM帧` → 解析 head+req | **11→16** |
| 2.7 | usrsvr | Redis + seqsvr | **无** | sid,uid,cid,nid → **`im:sid:*` + seq=50001** | — |
| 2.8 | usrsvr | **P-send** ACK | **Proxy** | **`MesgOnlineAck{seq,code}`** → RTMQ 0x0102 | **13→15** |
| 2.9 | BACKEND | **S-unicast** | **Server** | IM 帧 + **nid=20001** → ws-1 | **06→08→11** |
| 2.10 | ws-1 | **P-recv** → 浏览器 | **Proxy** | IM 帧 → WS + status=LOGIN | **16→17** |

详表 → [群聊故事线-逐步入参出参.md §3](群聊故事线-逐步入参出参.md#3-阶段-2websocket--online)
...
| 步 | 动作 | RTMQ | **入参 → 出参** | 17 篇 |
|----|------|------|-----------------|-------|
| 3.1 | 浏览器发 CREAT | **无** | **`MesgGroupCreat{uid,name,desc}`** + head | — |
| 3.2 | ws-1 **P-send** | **Proxy** | IM 帧 → RTMQ **0x0301** | **13-15** **02** |
| 3.3 | **S-pub** → usrsvr | **Server** | RTMQ → **16** handler | **10-12** **06** **09** |
| 3.4 | usrsvr 业务 | **无** | req → Redis **`gid=1`**, `chat:gid:1:to:nid` +20001 | — |
| 3.5 | **P-send** ACK | **Proxy** | **`MesgGroupCreatAck{errmsg:"Ok:1"}`** | **13-15** |
| 3.6 | **S-unicast** | **Server** | ACK + **nid=20001** → ws-1 | **06** **08** **11** |
| 3.7 | ws **P-recv** | **P+⚪** | ACK → **`ImGroupJoin(1,900001,1001)`** + WS | **16-17** |

详表 → [群聊故事线-逐步入参出参.md §4](群聊故事线-逐步入参出参.md#4-阶段-3100001-建群-group-creat)
...
| 步 | RTMQ | **入参 → 出参** | 17 篇 |
|----|------|-----------------|-------|
| groupBroadcast | **5× unicast** | 入：`GroupGetGidToNidSet(gid=1)` → `[20001..20005]`；出：每 nid 一包 **`MesgGroupJoinNtf{uid,gid}`** | **06** **08** |
| ws ImGroup | **无** | gid → **TravImGroupSession** → WS 帧 | — |

详表 → [群聊故事线-逐步入参出参.md §5](群聊故事线-逐步入参出参.md#5-阶段-4100002100010-加群)
...
### 7.1 逐步表（RTMQ + 入参出参 + 17 篇）

| # | 位置 | 动作 | RTMQ | **入参 → 出参** | 17 篇 |
|---|------|------|------|-----------------|-------|
| D1 | 浏览器 | GROUP_CHAT | **无** | **`MesgGroupChat{uid=100001,gid=1,text}`** | — |
| D2 | ws-1 | 补头 | **无** | head 补 cid/nid | — |
| D3 | P-send | 上行 | **Proxy** | IM 帧 → RTMQ **0x030B** | **13-15** |
| D4-D5 | publish | → msgsvr | **Server** | RTMQ → **16** `MsgSvrGroupChatHandler` | **10-12** **06** **09** |
| D7 | msgsvr | 校验 | **无** | `GroupGetRole(gid,uid)` | — |
| D8 | msgsvr | 读路由 | **无** | **`GroupGetGidToNidSet` → 5 nid** | — |
| D9-D11 | fan-out ① | 5× unicast | **Server×5** | **`send_data(..., nid=2000x, body=MesgGroupChat pb)`** ×5 | **06→08** |
| D12-D14 | fan-out ② | ImGroup | **无** | gid=1 → **10 浏览器 WS** 同一 IM 帧 | **16** |
| D15 | ACK | 1× uni | **P+S** | **`MesgGroupChatAck`** → nid=20001 | **13→08→16** |

**msgsvr `send_data` 关键入参**（与源码一致）：

```go
send_data(CMD_GROUP_CHAT, head.Sid, 0, targetNid, head.Seq, pbBytes, head.Length)
// targetNid 循环 20001..20005；body 为原始 MesgGroupChat protobuf（无 IM 头）
```

详表 → [群聊故事线-逐步入参出参.md §6](群聊故事线-逐步入参出参.md#6-阶段-5100001-发群消息--两段-fan-out)
