...
> **源文件**：`src/clang/lib/rtmq/server/rtmq_lsn.c`  
> **模块**：Server 监听与连接接入  
> **说明**：独立 listen 线程 accept → connq → pipe 唤醒 rsvr 线程 ADD_SCK。  
...
> **层级**：Server · 连接  
> **在 RTMQ 中的位置**：唯一 accept 线程：新 TCP 进 connq，唤醒 rsvr  
...
| 函数 | 约略位置 |
|------|----------|
| `rtmq_lsn_accept` | 见下方代码块 |
| `rtmq_add_sck_notify` | 见下方代码块 |
...
```c
// 文件: src/clang/lib/rtmq/server/rtmq_lsn.c
/******************************************************************************
...
/* 静态函数 */
/* ===== 阅读注释：rtmq_lsn_accept =====
 * 【新 TCP 连接】非阻塞 fd + connq push + RTMQ_CMD_ADD_SCK。
 */
